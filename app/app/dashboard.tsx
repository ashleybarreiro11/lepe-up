...
export default function Dashboard() {
  ...
  const { state, riseLabel, chargeLabel, chargeStartLabel, rotationNote, acceptSuggestion, dismissSuggestion } = useApp();
      ...
      <SafeAreaView style={{ flex: 1 }} edges={['top']}>
        <ScrollView contentContainerStyle={{ paddingHorizontal: 22, paddingTop: 12, paddingBottom: 8, gap: 22 }}>
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 12 }}>
            ...
            <StatCard icon={<IconBatteryBolt />} value={chargeStartLabel} label={`Inicio de carga · por ${chargeLabel}`} />
            <Pressable
              onPress={() => router.push('/settings')}
              style={{
                width: '100%',
